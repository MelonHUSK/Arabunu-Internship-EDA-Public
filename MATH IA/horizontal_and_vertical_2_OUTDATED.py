...
t_max = 60               # Max simulation time (s)
...
def euler_method(v_x0, v_y0, v_wind, dt, t_max):
    ...
    for i in range(1, len(t_values)):
        ...
        # Vertical drag force direction: Positive if projectile is falling, negative if rising
        drag_sign_y = -1 if v_y > 0 else 1

        # Update velocity (horizontal and vertical)
        dv_x = - (F_drag_x / m) * dt
        dv_y = (-g + drag_sign_y * F_drag_y / m) * dt
...
plt.title('Projectile Trajectory with Drag and Wind (Adjusted for Drag Direction)')
plt.grid(True)
plt.show()
